...
enum class COpType {
  Assign, Add, Sub, Mul, Div, Exp, Neg
};
...
struct Code : public Ref {
  ...
  void print(const std::list<Ref*>& code) {
    std::cout << addr(code) << ": " << opToStr(type) << " " << op1->addr(code) << ", " << op2->addr(code) << std::endl;
  }
  std::string addr(const std::list<Ref*>& code) {
    std::stringstream ss;
    ss << "$" << std::distance(code.cbegin(), std::find(code.cbegin(), code.cend(), this));
    return ss.str();
  }
};
